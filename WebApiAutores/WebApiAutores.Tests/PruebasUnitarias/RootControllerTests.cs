...
namespace WebApiAutores.Tests.PruebasUnitarias {
	...
	public class RootControllerTests {
		...
		public async Task SiUsuarioEsAdmin_Obtenemos4Links() {
			// Preparación
			var authorizationService = new AuthorizationServiceMock( AuthorizationResult.Success() );
		...
		public async Task SiUsuarioNoEsAdmin_Obtenemos2Links() {
			// Preparación
			var authorizationService = new AuthorizationServiceMock( AuthorizationResult.Failed() );
			var rootController = new RootController( authorizationService ) {
				Url = new URLHelperMock()
			};
